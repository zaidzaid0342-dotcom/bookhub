...
import { getAllUsers, deleteUser, getAllBooksWithoutFilter, adminDeleteBook } from '../services/api';
  ...
  useEffect(() => {
    if (users.length > 0 && books.length > 0) {
      const booksWithSellerInfo = books.map(book => {
        const seller = users.find(user => user.id === book.seller);
        return {
          ...book,
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden transition-transform duration-300 hover:shadow-lg hover:-translate-y-1">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold">
                    {books.reduce((acc, book) => acc + (book.offers ? book.offers.length : 0), 0)}
                  </p>
                </div>
            ...
            <div className="p-4 bg-white">
              <div className="h-2 bg-purple-100 rounded-full overflow-hidden">
                <div className="h-full bg-purple-500 rounded-full" style={{ width: `${Math.min(100, books.reduce((acc, book) => acc + (book.offers ? book.offers.length : 0), 0))}%` }}></div>
              </div>
            </div>
          </div>
        </div>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {booksWithSeller.map(book => (
                        <tr key={book._id} className="hover:bg-gray-50 transition-colors duration-150">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900">{book.seller ? book.seller.username : 'Unknown'}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
